...
@Component
export default class learningDoc extends Vue {
  ...
  getApi() {
    axios
      .get('api/file-storage/getAllFile/' + this.local.user.id + '/' + this.page)
      .then(res => {
        this.list = res.data;
  ...
  convertToSpace(val) {
    ...
    let space = (num / 1024).toFixed(2) + 'M';
    if (num / (1024 * 1024) >= 1) {
      space = (num / 1048576).toFixed(2) + 'G';
    }
    return space;
  }
  ...
  deleteAllOk() {
    axios
      .delete('api/file-storage/deleteAll/' + this.local.user.id + '/FILE')
      .then(res => {
        this.getApi();
      })
      .catch(error => {

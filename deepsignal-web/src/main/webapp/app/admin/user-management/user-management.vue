  ...
  <div class="jh-card">
    <h2>
      ...
      <div class="d-flex justify-content-end">
        <b-input-group class="mr-2" style="width: auto">
          <b-form-input placeholder="Search user ..." v-model="keyword" @keyup.enter="searchUser"></b-form-input>
          <b-input-group-append v-if="results">
        ...
        <router-link custom v-slot="{ navigate }" :to="{ name: 'DsUserCreate' }">
    ...
    <div class="table-responsive" v-if="users">
      <table class="table table-striped" aria-describedby="Users">
        ...
        <tbody v-if="users">
          <tr v-for="user in users" :key="user.id" :id="user.login">
            <!--            <td>-->
            <!--              <router-link :to="{ name: 'DsUserView', params: { userId: user.login } }">{{ user.id }}</router-link>-->
            <!--            </td>-->
            <td>
              <router-link :to="{ name: 'DsUserView', params: { userId: user.login } }">{{ user.login }}</router-link>
            </td>
            ...
            <td class="text-right">
              <div class="btn-group">
                <router-link :to="{ name: 'DsUserView', params: { login: user.login } }" custom v-slot="{ navigate }">
                  <button @click="navigate" class="btn btn-info btn-sm details">
                    <font-awesome-icon icon="eye"></font-awesome-icon>
                    <span class="d-none d-md-inline" v-text="$t('entity.action.view')">View</span>
                  </button>
                </router-link>
                <router-link :to="{ name: 'DsUserEdit', params: { login: user.login } }" custom v-slot="{ navigate }">
                ...
                <b-button
                  v-on:click="deleteUserByAdmin(user.id)"
                  variant="danger"
      ...
      <b-modal ref="removeUser" id="removeUser" v-bind:title="$t('entity.delete.title')" @ok="deleteUser()">
        <div class="modal-body">
          <p id="ds-delete-user-heading" v-text="$t('userManagement.delete.question', { login: removeId })">

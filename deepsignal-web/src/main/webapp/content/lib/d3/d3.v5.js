...
(function (global, factory) {
  ...
  function histogram() {
    ...
    histogram.thresholds = function (_) {
      return arguments.length
        ? ((threshold = typeof _ === 'function' ? _ : Array.isArray(_) ? constant(slice.call(_)) : constant(_)), histogram)
        : threshold;
    };
  ...
  function create$1(node, id, self) {
    ...
    function tick(elapsed) {
      var t =
          elapsed < self.duration ? self.ease.call(null, elapsed / self.duration) : (self.timer.restart(stop), (self.state = ENDING), 1),
        i = -1,
        n = tween.length;
  ...
  function transition_selectAll(select) {
    ...
    for (var groups = this._groups, m = groups.length, subgroups = [], parents = [], j = 0; j < m; ++j) {
      for (var group = groups[j], n = group.length, node, i = 0; i < n; ++i) {
        if ((node = group[i])) {
          for (
            var children = select.call(node, node.__data__, i, group), child, inherit = get$1(node, id), k = 0, l = children.length;
            k < l;
            ++k
          ) {
            if ((child = children[k])) {
  ...
  var elasticInOut = (function custom(a, p) {
    ...
    function elasticInOut(t) {
      return (
        ((t = t * 2 - 1) < 0 ? a * Math.pow(2, 10 * t) * Math.sin((s - t) / p) : 2 - a * Math.pow(2, -10 * t) * Math.sin((s + t) / p)) / 2
      );
    }
  ...
  function brush$1(dim) {
    ...
    function started() {
      ...
      var that = this,
        ...
        mode =
          (keys && exports.event.metaKey ? (type = 'overlay') : type) === 'selection'
            ? MODE_DRAG
            : keys && exports.event.altKey
            ? MODE_CENTER
            : MODE_HANDLE,
  ...
  function contours() {
    ...
    contours.thresholds = function (_) {
      return arguments.length
        ? ((threshold = typeof _ === 'function' ? _ : Array.isArray(_) ? constant$6(slice$3.call(_)) : constant$6(_)), contours)
        : threshold;
    };
  ...
  function density() {
    ...
    density.thresholds = function (_) {
      return arguments.length
        ? ((threshold = typeof _ === 'function' ? _ : Array.isArray(_) ? constant$6(slice$3.call(_)) : constant$6(_)), density)
        : threshold;
    };
  ...
  function formatLocale(locale) {
    ...
    function newFormat(specifier) {
      ...
      precision =
        precision === undefined ? 6 : /[gprs]/.test(type) ? Math.max(1, Math.min(21, precision)) : Math.max(0, Math.min(20, precision));
  ...
  function clipAntimeridianIntersect(lambda0, phi0, lambda1, phi1) {
    ...
    return abs(sinLambda0Lambda1) > epsilon$2
      ? atan(
          (sin$1(phi0) * (cosPhi1 = cos$1(phi1)) * sin$1(lambda1) - sin$1(phi1) * (cosPhi0 = cos$1(phi0)) * sin$1(lambda0)) /
  ...
  function transformer$3() {
    ...
    function scale(x) {
      return isNaN((x = +x))
        ? unknown
        : ((x = 0.5 + ((x = +transform(x)) - t1) * (x < t1 ? k10 : k21)), interpolator(clamp ? Math.max(0, Math.min(1, x)) : x));
    }
